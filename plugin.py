...
class PluginSectionConfig(PluginConfigBase):
    ...
    enabled: bool = Field(
        default=True,
        description="是否启用插件。",
        json_schema_extra=_ui_extra("启用插件", "关闭后不会处理 /codex 或 /agent 命令。", 1),
    )
...
class PermissionConfig(PluginConfigBase):
    ...
    admin_users: List[str] = Field(
        ...
        json_schema_extra=_ui_extra("管理员用户", "允许使用 danger-full-access 等高危本地 Codex 权限的用户。", 4, placeholder="qq:123456"),
    )
    ...
    trigger_chats: List[str] = Field(
        ...
        json_schema_extra=_ui_extra("聊天流黑白名单", "触发 /codex 的会话来源名单，可写群号、qq:群号 或 stream_id。", 7, placeholder="qq:群号"),
    )
...
class TaskConfig(PluginConfigBase):
    ...
    command_prefix: str = Field(
        ...
        json_schema_extra=_ui_extra("命令前缀", "帮助文本里默认展示的主命令。实际命令固定支持 /codex 和 /agent。", 0),
    )
    execution_mode: str = Field(
        default="local",
        description="执行模式：local 或 remote。",
        json_schema_extra=_ui_extra("执行模式", "local 直接调用本机 Codex CLI；remote 调用远程 HTTP Agent。", 1, placeholder="local / remote"),
    )
...
class LocalCodexConfig(PluginConfigBase):
    ...
    codex_binary: str = Field(
        default="codex",
        description="Codex CLI 可执行文件名或绝对路径。",
        json_schema_extra=_ui_extra("Codex 可执行文件", "Codex CLI 命令名或绝对路径，例如 codex 或 C:/Users/你/AppData/Roaming/npm/codex.cmd。", 0),
    )
    work_root: str = Field(
        default="data/tasks",
    ...
    sandbox: str = Field(
        ...
        json_schema_extra=_ui_extra("沙箱模式", "传给 Codex CLI 的 -s 参数。服务器无人值守建议使用 workspace-write。", 2, placeholder="workspace-write"),
    )
    approval_policy: str = Field(
        default="never",
        description="Codex CLI 审批策略。",
        json_schema_extra=_ui_extra("审批策略", "传给 Codex CLI 的 -a 参数。服务器无人值守建议使用 never。", 3, placeholder="never"),
    )
    model: str = Field(
        default="",
        description="可选模型名。",
        json_schema_extra=_ui_extra("模型名", "传给 Codex CLI 的模型名。留空时使用 Codex CLI 默认配置。", 4),
    )
    enable_search: bool = Field(
        default=False,
        description="是否启用 Codex CLI --search。",
        json_schema_extra=_ui_extra("启用搜索", "是否给 Codex CLI 传递 --search。需要当前 Codex CLI 版本支持。", 5),
    )
    extra_args: List[str] = Field(
        default_factory=list,
        description="额外传给 codex exec 的参数。",
        json_schema_extra=_ui_extra("额外参数", "追加传给 codex exec 的参数，例如 --profile server-agent。", 6),
    )
    pass_env_vars: List[str] = Field(
        default_factory=list,
        description="额外传给 Codex 子进程的环境变量名；这些变量可能被 Codex、skill 或 MCP 读取。",
        json_schema_extra=_ui_extra("传递环境变量", "默认只传最小运行环境。只有确认需要且能接受泄露风险时，才把 API key/token 变量写在这里。", 7, placeholder="OPENAI_API_KEY"),
    )
    process_timeout_seconds: float = Field(
        default=3600.0,
        description="本地 Codex 任务运行超时。",
        json_schema_extra=_ui_extra("进程超时", "本地 Codex 子进程最长运行时间，单位秒。", 8, step=60),
    )
    artifact_globs: List[str] = Field(
        default_factory=lambda: ["artifacts/*", "*.docx", "*.pdf", "*.md", "*.zip", "*.xlsx", "*.pptx"],
        description="任务 workspace 内产物匹配规则。",
        json_schema_extra=_ui_extra("产物匹配规则", "任务结束后在 workspace 内扫描这些 glob，匹配到的文件会作为产物回传。", 9, placeholder="artifacts/*"),
    )
...
class ProgressConfig(PluginConfigBase):
    ...
    forward_progress: bool = Field(
        ...
        json_schema_extra=_ui_extra("转发运行进度", "是否把 Codex CLI 的阶段性进度发送回 QQ。", 0),
    )
    ...
    max_progress_items_per_message: int = Field(
        ...
        json_schema_extra=_ui_extra("单条消息进度数", "每次进度消息最多合并多少条 Codex 进度。", 2),
    )
    ...
    private_progress_trigger_args: List[str] = Field(
        ...
        json_schema_extra=_ui_extra("私聊进度参数", "用户在 /codex 命令中添加这些参数时，会尝试私聊发送阶段性进度。", 6, placeholder="--dm"),
    )
...
class InputFileConfig(PluginConfigBase):
    ...
    enable_reply_file: bool = Field(
        default=True,
        description="是否允许回复 QQ 文件消息创建带材料的 Codex 任务。",
        json_schema_extra=_ui_extra("允许回复文件作为输入", "用户回复 QQ 文件消息并发送 /codex 时，插件会把文件导入 workspace/input。", 0),
    )
    input_dir_name: str = Field(
        default="input",
        description="输入材料放入 workspace 下的目录名。",
        json_schema_extra=_ui_extra("输入目录名", "输入材料在 Codex workspace 下的目录名。", 1),
    )
    max_files_per_task: int = Field(
        default=5,
        description="单个任务最多导入多少个文件。",
        json_schema_extra=_ui_extra("最多导入文件数", "一个 /codex 任务最多导入多少个被回复或附带的文件。", 2),
    )
    max_file_size_mb: float = Field(
        default=100.0,
...
class RemoteCodexAgentConfig(PluginConfigBase):
    ...
    permission: PermissionConfig = Field(
        ...
        description="触发权限配置。",
        json_schema_extra=_ui_extra("权限", "控制哪些用户和聊天流可以触发 /codex。", 4),
    )
    ...
    local_codex: LocalCodexConfig = Field(
        default_factory=LocalCodexConfig,
        description="本机 Codex CLI 配置。",
        json_schema_extra=_ui_extra("本地 Codex", "local 执行模式下调用 Codex CLI 的配置。", 6),
    )
    ...
    input_file: InputFileConfig = Field(
        ...
        description="输入文件配置。",
        json_schema_extra=_ui_extra("输入文件", "回复 QQ 文件作为 Codex 输入材料的配置。", 9),
    )
